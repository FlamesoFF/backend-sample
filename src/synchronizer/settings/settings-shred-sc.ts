const settings = {
    processors: [{
        enable: true,
        name: 'shred',
        condition: (change) => change.doc && (change.doc.type && change.doc.type.includes('todo') && change.doc.type.includes('shred') && change.doc.authority == 'Seychelles'),
        script: './scripts/shredder-sc.ts',
        defaults: {
            payloads: [{
                enable: true,
