const settings = {
    processors: [{
        enable: true,
        name: 'shred',
        condition: (change) => change.doc && (change.doc.type && change.doc.type.includes('todo') && change.doc.type.includes('shred')),
        script: './scripts/shredder.ts',
        defaults: {
            payloads: [{
                enable: true,
                condition: (todo) => todo.completed && todo.tags && todo.tags.includes('check-file-content') && !(todo.comments || []).some(c => c.body.trim().toLowerCase() == 'not found'),
                body: {
                    type: ['todo', 'shred'],
                ...
                enable: true,
                condition: (todo) => todo.completed && todo.tags && todo.tags.includes('shred-file') && (todo.comments || []).some(c => c.body.trim().toLowerCase() == 'to assess'),
                body: {
                    type: ['todo', 'shred'],
                    content: 'Move to CD',
                    creator_guid: 'm.sereda',
                ...
                {
                    enable: true,
                    condition: (todo) => todo.completed && todo.tags && todo.tags.includes('ca-move-cm') && !(todo.comments || []).some(c => c.body.trim().toLowerCase() == 'shredding approved'),
                    body: {
                        type: ['todo', 'shred'],
                        content: 'File to FC',
