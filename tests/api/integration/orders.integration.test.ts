...
import { Utils } from '../../../src/api/shared/utils';
import { ajv, getAddress, gotInstance } from './integration.test';

describe('/v3/orders', () => {
    let orderId: string;
    const managerId = 'person-integration-test';
    const clientId = 'company-integration-test';
    ...
    it('POST /v3/orders', async () => {
        ...
        const {
            statusCode,
            body: { id }
        } = await gotInstance.post(`${getAddress()}/v3/orders`, {
            json: payload,
            responseType: 'json'
        });
    ...
    it('GET /v3/orders', async () => {
        const { statusCode, body } = await gotInstance.get(`${getAddress()}/v3/orders`, {
            responseType: 'json'
        });
    ...
    it('GET /v3/orders?manager_id=<managerId>', async () => {
        const { statusCode, body } = await gotInstance.get(`${getAddress()}/v3/orders?manager_id=${managerId}`, {
            responseType: 'json'
        });

        expect(statusCode).to.be.equal(200);

        const keys = (<any[]>body).map(item => item._id);
    ...
    it('GET /v3/orders/<id>', async () => {
        const { statusCode, body } = await gotInstance.get(`${getAddress()}/v3/orders/${orderId}`, {
            responseType: 'json'
        });
    ...
    it('DELETE /v3/orders/<id>', async () => {
        const { statusCode } = await gotInstance.delete(`${getAddress()}/v3/orders/${orderId}`, {
            responseType: 'json'
        });
        ...
        try {
            await gotInstance.get(`${getAddress()}/v3/orders/${orderId}`);
        } catch (error) {
